...
func parseResponse(content string) (int, string, error) {
	// Padrão para extrair: ID: 1, Nome: Consulta Limite / Vencimento do cartão / Melhor dia de compra
	re := regexp.MustCompile(`ID:\s*(\d+),?\s*Nome:\s*(.+?)(?:\n|$)`)
	matches := re.FindStringSubmatch(content)

	if len(matches) < 3 {
		// Tentar formato alternativo: service_id: 1
		re2 := regexp.MustCompile(`service_id:\s*(\d+)`)
	...
	if err != nil {
		return 0, "", fmt.Errorf("ID de serviço inválido: %s", matches[1])
	}
